...
import { caseStudiesAPI, servicesAPI, blogsAPI, jobsAPI, usersAPI } from "./api-service"
...
export function DataProvider({ children }: { children: React.ReactNode }) {
    ...
    const addUser = async (user: Omit<User, "id">) => {
        try {
            const res = await usersAPI.create({
                ...
                email: user.email,
                password: "", // Password should be provided in the form
                role: user.role,
            ...
            if (res.success && res.user) {
                const newUser: User = {
                    ...
                    image: res.user.image?.imageLink || "",
    ...
    const updateUser = async (id: string, updatedUser: Partial<User>) => {
        try {
            ...
            const apiData: any = {}
            if (updatedUser.userName) apiData.userName = updatedUser.userName
            if (updatedUser.email) apiData.email = updatedUser.email
            if (updatedUser.role) apiData.role = updatedUser.role
            if (updatedUser.isActive !== undefined) apiData.isActive = updatedUser.isActive
